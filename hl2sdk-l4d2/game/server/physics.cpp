...
CON_COMMAND( physics_budget, "Times the cost of each active object" )
{
	...
	if ( activeCount )
	{
		...
		float full = engine->OBSOLETE_Time();
		physenv->Simulate( DEFAULT_TICK_INTERVAL );
		full = engine->OBSOLETE_Time() - full;
		float lastTime = full;
		...
		for ( i = 0; i < ents.Count(); i++ )
		{
			...
			float start = engine->OBSOLETE_Time();
			physenv->Simulate( DEFAULT_TICK_INTERVAL );
			float end = engine->OBSOLETE_Time();
...
void PhysFrame( float deltaTime )
{
	...
	if ( bProfile )
	{
		simRealTime = engine->OBSOLETE_Time();
	}
	...
	if ( bProfile )
	{
		simRealTime = engine->OBSOLETE_Time() - simRealTime;
