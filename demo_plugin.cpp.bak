...
IGameEventManager2* gameeventmanager_ = NULL; // game events interface
#ifndef GAME_DLL
...
{
    Msg("CEmptyServerPlugin::Load start\n");
	...
	if (!playerinfomanager) {
        ...
        Msg("IPlayerInfoManager::playerinfomanager found at %d\n", playerinfomanager);
    }
	...
	if (!botmanager) {
        ...
        Msg("IBotManager::botmanager found at %d\n", botmanager);
    }
    ...
    if (!engine) {
		Warning("engine is null\n");
    } else {
        Msg("IVEngineServer::engine found at %d\n", engine);
    }
	gameeventmanager = (IGameEventManager2*)interfaceFactory(INTERFACEVERSION_GAMEEVENTSMANAGER2, NULL);
	if (!gameeventmanager) {
		Warning("gameeventmanager is null\n");
    } else {
        Msg("IGameEventManager2::gameeventmanager found at %d\n", gameeventmanager);
    }

	helpers = (IServerPluginHelpers*)interfaceFactory(INTERFACEVERSION_ISERVERPLUGINHELPERS, NULL);
	if (!helpers) {
		Warning("helpers is null\n");
    } else {
        Msg("IServerPluginHelpers::helpers found at %d\n", helpers);
    }

	enginetrace = (IEngineTrace*)interfaceFactory(INTERFACEVERSION_ENGINETRACE_SERVER, NULL);
	if (!enginetrace) {
		Warning("enginetrace is null\n");
    } else {
        Msg("IEngineTrace::enginetrace found at %d\n", enginetrace);
    }

	randomStr = (IUniformRandomStream*)interfaceFactory(VENGINE_SERVER_RANDOM_INTERFACE_VERSION, NULL);
	if (!randomStr) {
		Warning("randomStr is null\n");
    } else {
        Msg("IUniformRandomStream::randomStr found at %d\n", randomStr);
    }
	...
	ConVar_Register(0);
    Msg("CEmptyServerPlugin::Load end\n");
	return true;
}
...
void CEmptyServerPlugin::LevelInit(char const* pMapName)
{
    ...
    gameeventmanager->AddListener(this, "player_jump_apex", true);
...
int CEmptyServerPlugin::GetEventDebugID(void)
{
    //很神秘,需要返回42才行,否则控制台返回GameEventListener2 callback in list that should NOT be - xxx(事件名)!
    //解决办法参考https://forums.alliedmods.net/showthread.php?t=194382
    return 42;
}
//---------------------------------------------------------------------------------
